...
int main(int argc, char **argv)
{
    ...
    MPI_Type_create_subarray(2, sizes, subsizes, offsets, MPI_ORDER_C, MPI_INT, &subblock);
    MPI_Type_commit(&subblock);
    
    // Print out matrix of rank 0.
    if (rank == 0) {
        for (i = 0; i < 8; i++) {
            for (j = 0; j < 8; j++) {
                printf("%3d", array[i][j]);
    ...
    // rank 0 to rank 1
    if (rank==0) {
    MPI_Send(array, 1, subblock, 1, 666, MPI_COMM_WORLD);
    } else if (rank==1) {
    MPI_Recv(array, 1, subblock, 0, 666, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
